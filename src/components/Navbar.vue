  ...
  <nav class="navbar navbar-expand-lg bg-primary">
    <div class="container">
      <div class="d-flex align-items-center">
        <!-- menu-bar -->
        <a
          class="d-lg-none d-block text-white fs-3 pe-4" data-bs-toggle="collapse"
      ...
      <!-- search -->
      <div ref="searchSection" class="search-wrap order-2 order-sm-1 ms-auto" @mouseleave="handleMouseLeave">
        <input
      ...
      <div class="nav-item dropdown login-dropdown order-1 order-sm-2 d-flex align-items-center">
        ...
        <ul ref="dropdownMenuLogin" class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <li class="border-bottom">
            <router-link
              :to="{ name: 'OrderHistory', params: { memberID: memberDataId } }"
              class="dropdown-item">
              訂單記錄
            </router-link>
          </li>
          ...
          <li class="border-bottom">
            <router-link
              :to="{ name: 'MyCollection', params: { memberID: memberDataId } }"
              class="dropdown-item">
              我的收藏
            </router-link>
          </li>
          <li class="border-bottom">
            <router-link
              :to="{ name: 'MyTicket', params: { memberID: memberDataId } }"
              class="dropdown-item">
              我的票券
            </router-link>
          </li>
          <li class="border-bottom">
            <router-link
              :to="{ name: 'TicketList', params: { memberID: memberDataId } }"
              class="dropdown-item">
              購票清單
            </router-link>
          </li>
  ...
  methods: {
    ...
    sendSearch() {
      ...
      if (localStorage.getItem('searchHistory')) {
        ...
        const index = historyArray.indexOf(this.searchData);
        //如果此搜尋值已存在陣列之中，則刪除該值再將這個值塞到最後一位
        if (index !== -1) {
